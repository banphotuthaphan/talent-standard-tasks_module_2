    ...
    public class ProfileController : Controller
    {
        ...
        public async Task<IActionResult> GetTalentSnapshots(FeedIncrementModel feed)
        {
            try
            {
                ...
                var employerProfile = await _employerRepository.GetByIdAsync(_userAppContext.CurrentUserId);

                return Json(new { Success = true, Data = result, Employer = employerProfile });
            }
            catch (Exception e)
            {
